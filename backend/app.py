...
#     # Consumers can self-register; creators are admin-seeded
...
#         Image.caption.ilike(f"%{q}%") |
#         Image.location.ilike(f"%{q}%") |
#         Image.people.ilike(f"%{q}%")
...
# def add_comment():
#     data = request.json
#     if not data.get("image_id") or not data.get("text", "").strip():
#         return jsonify({"message": "image_id and text required"}), 400
...
#         return jsonify({"message": "image_id and value required"}), 400
...
#     else:
#         rating = Rating(image_id=image_id, user_id=request.user_id, value=int(value))
...
# # ─────────────────────────────────────────────
# #  ADMIN — seed creator (no public UI)
# # ─────────────────────────────────────────────

# @app.route("/admin/seed-creator", methods=["POST"])
# def seed_creator():
#     """Internal endpoint to create creator accounts — no public UI"""
#     data = request.json
#     secret = data.get("admin_secret", "")
#     if secret != "pixshare-admin-2024":
...
#     hashed = generate_password_hash(password)
#     creator = User(username=username, password=hashed, role="creator")
#     db.session.add(creator)
#     db.session.commit()
#     return jsonify({"message": f"Creator '{username}' created"}), 201
...
#         db.create_all()
#         # Seed a default creator if none exists
#         if not User.query.filter_by(role="creator").first():
#             hashed = generate_password_hash("creator123")
#             creator = User(username="creator", password=hashed, role="creator")
#             db.session.add(creator)
#             db.session.commit()
#             print("✅ Default creator seeded: username=creator, password=creator123")
